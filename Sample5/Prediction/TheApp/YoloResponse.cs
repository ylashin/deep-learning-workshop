...
namespace TheApp
{
    public class YoloResponse
    {
        ...
        public YoloResponse(IList<float> input)
        {
            ...
            var gridItemCount = 13 * 13;
            var skip = 0;
            for (var i = 0; i < 125; i++)
            {
                var data = input.Skip(skip).Take(gridItemCount).ToList();
                skip += gridItemCount;
                // data is 13 * 13 byte array
                ...
                for (var a = 0; a < 13; a++)
                {
                    ...
                    {
                        // IMPORTANT
                        // The index swap below is by trial and error :)
                        if (!Blocks.ContainsKey((b, a)))
                            Blocks[(b, a)] = new Block(b, a);

                        Blocks[(b, a)].SetEntry(i, data[index++]);
                    }
                }
            }
